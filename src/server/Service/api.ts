...
export const responsePlayList = (req: express.Request, res: express.Response, list: PLayList): void => {
    ...
    let disposition = /firefox|Firefox/.test(<string>req.headers['user-agent']) ? 'inline' : 'attachment';
...
export const responseFile = (req: express.Request, res: express.Response, filePath: string, mime: string, download: boolean = false): void => {
    let stat: fs.Stats;

    stat = fs.statSync(filePath)
    if(stat.isDirectory()) { throw new Error('file path is derectory'); }
...
const readRangeHeader = (range: string | string[] | undefined | null, totalLength: number): { Start: number, End: number } | null => {
...
const sendResponse = (code: number, req: express.Request, res: express.Response, responseHeaders: {}, readable: fs.ReadStream | null): void => {
    res.status(code);
