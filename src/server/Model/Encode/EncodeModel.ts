...
class EncodeModel extends Model implements EncodeModelInterface {
    ...
    constructor(
        ...
        this.encodeManager.addListener((id, name, filePath, delTs) => { this.encodeFinCallback(id, name, filePath, delTs); });
    }
    ...
    */
    private async encodeFinCallback(recordedId: number, name: string, filePath: string, delTs: boolean): Promise<void> {
        try {
            // エンコード済みファイルを DB へ追加
