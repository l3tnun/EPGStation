    ...
    public async set(tsPath: string, readableStream: stream.Readable): Promise<void> {
        ...
        tsPacketAnalyzer.on('finish', async() => {
            ...
            for (const pid of Object.keys(result)) {
                ...
                await this.appendFile(`pid: ${ this.pidToString(pidNum) }, error: ${ result[pid].error }, drop: ${ result[pid].drop }, scrambling: ${ result[pid].scrambling }, packet: ${ result[pid].packet }, name: ${ this.getPIDName(pidNum) }\n`);
            }
        });
